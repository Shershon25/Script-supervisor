...
- Characters (type: "character"): MUST be actual named individual people or sentient beings appearing or referenced in the screenplay (e.g., "Maya Reyes", "Elias Reyes", "Daniel Kerr", "Nora Chen").
  ...
  - NEVER extract generic off-screen speaking descriptors, V.O./O.S. dialogue headers, PA systems, speakers, or audio channels as character entities (e.g. ❌ "Radio Voice", ❌ "Voice Over", ❌ "V.O.", ❌ "O.S.", ❌ "PA System", ❌ "Speaker", ❌ "Audience", ❌ "Camera", ❌ "Narrator"). Do NOT extract "Radio Voice" or any similar audio channel descriptor as a character.
  - NEVER extract capitalized screenplay action words, weather, natural phenomena, or environmental events as characters (e.g. ❌ "flood", ❌ "fire", ❌ "explosion", ❌ "storm", ❌ "rain", ❌ "water"). "flood" is an environmental hazard, NEVER a character.
...
7. Respect fictional world rules (HIGHEST PRIORITY & SEMANTIC SUBSUMPTION):
   - Fictional World Rules defined in the screenplay or story state take 100% precedence over real-world physics, logic, scientific norms, or standard timeline logic.
   - GENERALIZED WORLD RULES: If a world rule is broadly phrased or generalized (e.g. "The research sphere generates a stable temporal field that alters time around objects, projects distorted imagery, and exhibits non-standard physics upon collapse"), evaluate semantic subsumption: any candidate event, object behavior, image distortion, or time anomaly covered under the broad concept/scope of that rule MUST be subsumed and CLASSIFIED AS NO_CONFLICT (or marked as permitted/intentional). Do not require exact phrasing match if the behavior falls under the generalized principle.
   - SPECIFIC WORLD RULES: Conversely, if a world rule is narrowly specified (e.g. "The temporal field only affects copper objects within 5 meters"), apply it STRICTLY to that specific condition and DO NOT generalize it to other unmentioned cases (e.g. an anomaly affecting wooden items or distant objects is still a candidate CONFLICT).
   - An action, clock movement, or physics anomaly permitted by an established Fictional World Rule MUST BE CLASSIFIED AS NO_CONFLICT.
