...
def test_day2_5_scene_acceptance_scenario(client):
    ...
    assert "story_state" in res1.json()
    ...
    assert john_state["residence"] == "Chennai"
    assert john_state["current_location"] is not None
    assert "mumbai" in john_state["current_location"]["name"].lower()
